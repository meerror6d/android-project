...
public class HomeFragment extends Fragment {
    ...
    private homepageadaptar homepageadaptar;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,ViewGroup container,Bundle savedInstanceState) {
       ...
       homepafemodelList.add(new homepafemodel(3,"Trending!",horizontalproductscrollmodellist));*/
        homepageadaptar=new homepageadaptar(homepafemodelList);
                            ...
                            for(QueryDocumentSnapshot documentSnapshot: task.getResult()){
                                ...
                                if((long)documentSnapshot.get("view_type")==0){
                                    ...
                                    for(long x=1;x<no_of_banners+1;x++){
                                        slidermodelList.add(new slider_model(documentSnapshot.get("banner_"+x).toString(),
                                                documentSnapshot.get("banner_"+x+"_back").toString()));
                                    }
                                    homepafemodelList.add(new homepafemodel(0,slidermodelList));
                                }
                                else if((long)documentSnapshot.get("view_type")==1){
                                    homepafemodelList.add(new homepafemodel(1,documentSnapshot.get("strip_ad_banner").toString(),
                                    ...
                                    for(long x=1;x<no_of_products+1;x++){
                                            horizontalscrollmodelList.add(new horizontalscrollmodel(documentSnapshot.get("product_id_"+x).toString(),
                                                    documentSnapshot.get("product_image_"+x).toString(),
                                                    documentSnapshot.get("procuct_title_"+x).toString(),
                                                    documentSnapshot.get("product_subtitle_"+x).toString(),
                                                    documentSnapshot.get("product_price_"+x).toString()));
                                    }
                                    homepafemodelList.add(new homepafemodel(2,documentSnapshot.get("layout_title").toString(),horizontalscrollmodelList));
