package com.Sornaly.gadgetstore;
...
import com.Sornaly.gadgetstore.ui.my_cart.MyCartFragment;
import com.Sornaly.gadgetstore.ui.my_mall.MyMallFragment;
import com.Sornaly.gadgetstore.ui.my_orders.MyOrdersFragment;
import com.Sornaly.gadgetstore.ui.my_rewards.MyRewardsFragment;
import com.Sornaly.gadgetstore.ui.my_wishlist.MyWishlistFragment;
...
public class DBquerries {
    ...
    //recyclerview er jnno......
    public static void loadCategories(final RecyclerView categoryRecyclerView, final Context context){

        categoryModelList.clear();

        firebaseFirestore.collection("Recyclerview").orderBy("index").get()
    ...
    //banner,strip ad,horizontal,grid product...........
    public static void loadFragmentData(final RecyclerView myMallRecyclerView, final Context context, final int index, String categoryName){
        ...
        firebaseFirestore.collection("Recyclerview")
                .document(categoryName.toUpperCase())
                .collection("PROMOTION").orderBy("index").get()
                .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if(task.isSuccessful()){
                            for(QueryDocumentSnapshot queryDocumentSnapshot : task.getResult()){
                                ...
                                if((long)queryDocumentSnapshot.get("view_type") == 0){
                                    ...
                                    for(long x=1;x<=no_of_banners;x++){
                                        sliderModelList.add(new SliderModel(queryDocumentSnapshot.get("banner_"+x).toString(),queryDocumentSnapshot.get("banner_"+x+"_back").toString()));
                                    }
                                ...
                                //grid view product.......
                                    ...
                                    for(long x=1;x<=no_of_products;x++){
                                        gridProductScrollModelList.add(new HorizontalProductScrollModel(
                                                queryDocumentSnapshot.get("product_ID_"+x).toString()
                                                ,queryDocumentSnapshot.get("product_image_"+x).toString()
                                                ,queryDocumentSnapshot.get("procuct_title_"+x).toString()
                                                ,queryDocumentSnapshot.get("product_subtitle_"+x).toString()
                                                ,queryDocumentSnapshot.get("product_price_"+x).toString()
                                        ));
                                    }
